...
> **Update (2026-07-11):** the landing page pivoted away from a dark cinematic
> noir site. The look is now a **warm paper-and-cards system** (the *logic* of
> PostHog's design system, not its brand) with pixel-art techniques layered on
> top, expressed literally as a pixel-art detective who walks around the page.
> The dark noir canvas, full-page grain, and vignette are GONE from the landing
> page — near-black now lives only inside contained "screen" panels. Built in
> `app/landing.css` (isolated from `globals.css`); `DESIGN.md` is the PostHog
> structural reference. The in-game pages (`/station`, `/interrogation`) keep
> their original dark noir identity.

The game is 1940s film-noir police-procedural. The landing page renders the same
world as a **case file on a desk** — cream paper, flat bordered cards, red
string, rubber stamps — not a generic SaaS page and not a dark cinematic site.

**Palette (exact, do not substitute):**
- Canvas (page bg, warm aged paper): `#ece0c3`
- Card surface (index-card stock): `#f8f2e2`
- Ink (headlines, high-contrast): `#2a2015`
- Body text: `#57493a`
- Muted (captions, timestamps): `#8c7d64`
- Hairline border (cards, dividers): `#c9b78e`
- Accent — stamp red (the ONE saturated color: CTA pill, rubber stamps, red
  string): `#b3271b`, pressed `#8f1f15`. Nothing else is saturated.
- Amber tag (rare secondary: folder tabs, evidence-tag borders — never a CTA):
  `#c98a2e`
- Screen surface (ONLY for the demo video, Memory ON/OFF proof, terminal
  snippet): `#16130f`, with screen text `#e8dfc8`

**Type:**
- Display / headlines: **Bebas Neue** (kept — reads like a stamped report header)
- Body / UI / case-file text: **IBM Plex Mono** (kept — typewriter on cream =
  police report; better for this brand than a friendly sans)
- Tiny stamped labels only (case numbers, timestamps): **Silkscreen**, small,
  used like rubber-stamp ink

**Card chrome (PostHog discipline):** flat surface, 1px hairline border, 6–8px
radius, **no drop shadows**, 20–24px padding. One feature-card variant anchors a
small pixel-art suspect/evidence icon in the top corner (the system's signature
card detail — not a shadow or gradient).

**Paper-vs-screen split (the page's visual argument):** most of the page is
"paper" (cream canvas + cards). A few elements are "screen" content and get the
dark screen surface instead — the demo video, the Memory ON/OFF proof, and the
terminal setup snippet. ONLY those get scanlines, monitor glow, and grain; the
rest stays flat and paper-like. Don't blur the contrast.

**Signature element (the one thing the page gets remembered for):** the hero is a
desk with a case file and a small corkboard of pinned evidence photos connected
by red string (the "memory as a lens / connecting everything" metaphor), rendered
in the cream/paper system. A small pixel-art detective sprite (from the existing
`public/sprites/detective.png`) physically walks across the scene and continues
to walk the page as you scroll, hopping when you cross into a new section and
flipping to face its direction of travel. Disabled under `prefers-reduced-motion`.

**Pixel techniques (copy the techniques, not a specific site's execution):**
pixel-block load/scroll reveal (stepped opacity), a custom pixel magnifier cursor
(the visitor is the investigator), a redaction-flash sweep on hovering evidence
tags, and scanlines confined to screen panels.

**Don't:** the exact PostHog palette/wordmark/hedgehog; warm cream + high-contrast
serif + terracotta accent (a different generic AI cluster — avoid even though the
canvas is also warm); purple-on-white; startup gradient blobs; glassmorphism;
rounded pastel cards with drop shadows. One saturated accent only.
