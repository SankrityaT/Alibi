...
export interface UseSpokenLineResult {
  ...
  /** Playback speed applied to both the audio element and browser TTS fallback. */
  playbackRate: number
  /** Change the playback speed; takes effect on the next (or current) line. */
  setPlaybackRate: (rate: number) => void
  /** Immediately stop whatever is currently speaking (audio or browser voice). */
  stop: () => void
}
...
export function useSpokenLine(
  ...
  const speakViaBrowser = useCallback((text: string, suspectId?: string): boolean => {
    ...
    try {
      ...
      utterance.rate = 0.98 * playbackRateRef.current
  ...
  // Stop whatever is currently speaking — used by the "skip" control so a
  // slow line can be cut short instead of waited out.
  ...
  return { speak, isSpeaking, ttsAvailable, playbackRate, setPlaybackRate, stop }
}
