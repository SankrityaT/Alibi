...
import { ThinkingIndicator } from '../../../components/interrogation/ThinkingIndicator.js'
...
export default function InterrogationPage({ params }: InterrogationPageProps) {
  ...
  const { speak, isSpeaking, ttsAvailable, playbackRate, setPlaybackRate, stop: stopSpeaking } =
    useSpokenLine()
  ...
  useEffect(() => {
    // Scroll only the transcript's own scroll container (not scrollIntoView,
    // which walks up and can drag the whole page/body along with it — that
    // was pushing the HUD off-screen every time a new answer landed).
    const container = transcriptRef.current
    // Optional-call: jsdom (tests) doesn't implement scrollTo.
    container?.scrollTo?.({ top: container.scrollHeight, behavior: 'smooth' })
  }, [turns])

  useEffect(() => {
    // This scene is a fixed-viewport stage with its own internal scroll
    // region (the transcript). Lock the page body so nothing — a stray
    // scrollIntoView, a resize, a focus jump — can scroll the document
    // itself and shove the HUD off-screen.
  ...
  async function askQuestion(text: string) {
    ...
    setIsLoading(true)
    setError(null)
    // Show the question immediately with a "thinking" placeholder in place of
    // the answer, instead of leaving the transcript looking frozen until the
    // whole response comes back.
  ...
  return (
    <main className="scene">
      ...
      <div className="hud">
        ...
        <span style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <button
            ...
            className="hud__btn"
            title="Cycle dialogue/voice speed"
          >
            {playbackRate}&times; Speed
          </button>
          <button
            type="button"
            onClick={handleSkip}
            disabled={!isSpeaking && !speakingViz}
            aria-label="Skip to end of line"
            className="hud__btn"
            title="Skip the rest of this line"
          >
            Skip &raquo;
          </button>
          <button
      ...
      <div className="console">
        <div className="console__transcript" data-testid="transcript" ref={transcriptRef}>
          {latestTurn || pendingQuestion ? (
            <>
              {turns.map((turn, i) => (
                <div key={turn.id} className="console__turn">
                  ...
                  {i === turns.length - 1 && !pendingQuestion ? (
                    <DialogueBox
                      text={turn.answer}
              ...
              {pendingQuestion && (
                <div className="console__turn" data-testid="pending-turn">
                  <p className="dialogue-scene__you">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img className="dlg-avatar" src={INVESTIGATOR_SPRITE} alt="" aria-hidden="true" />
                    You: {pendingQuestion}
